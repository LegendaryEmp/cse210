class GoalManager
{
    ...
    public void CreateNewGoal(string goalType)
    {
        ...
        {
            Console.WriteLine("Invalid input. Please enter an integer for points to be awarded.");
        }
        ...
        switch (goalType)
        {
            ...
            case "3":
                ...
                while (!int.TryParse(Console.ReadLine(), out timesToAccomplish) || timesToAccomplish <= 0)
                {
                    Console.WriteLine("Invalid input for times to accomplish. Please enter a positive integer.");
                }
                AddActivity(new ChecklistGoal(goalName, goalDescription, pointsAwarded, timesToAccomplish));
                break;
            default:
        ...
        foreach (Activity activity in activities)
        {
            if (activity.Name == goalName)
            {
                ...
                if (activity is SimpleGoal simpleGoal)
                {
                    ...
                    Console.WriteLine("");
                    Console.WriteLine($"Congratulations! You have earned {pointsEarned} points for completing a Simple Goal.");
                }
                else if (activity is EternalGoal eternalGoal)
                {
                    ...
                    availablePoints += pointsEarned;
                    Console.WriteLine("");
                    Console.WriteLine($"Congratulations! You have earned {pointsEarned} points for completing an Eternal Goal.");
                }
                else if (activity is ChecklistGoal checklistGoal)
                {
                    ...
                    if (checklistGoal.IsCompleted())
                    {
                        ...
                        availablePoints += pointsEarned;
                        Console.WriteLine("");
                        Console.WriteLine($"Congratulations! You have earned {pointsEarned} points for completing a Checklist Goal.");
                    }
                }
                found = true;
                break;
